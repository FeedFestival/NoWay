...
public class Sphere : MonoBehaviour
{
    ...
    public void RollArround(Dir dir, float percent = 4f, float? animT = null, bool reverse = false)
    {
        ...
        switch (dir)
        {
            case Dir.Up:
                ...
                break;
            case Dir.Right:
                vDir = reverse ? Vector3.forward : -Vector3.forward;
                break;
            case Dir.Down:
                vDir = reverse ? -Vector3.left : Vector3.left;
                break;
    ...
    private bool ValidateNewPos(int x, int y)
    {
        ...
        // this one fills [ bool noMove ] so true means no movement !!!

        if (TileState.Blocked == tile.State)
            return true;
        ...
        if (tile.State == TileState.BoxIn)
        {
            ...
            var boxObj = Main.Instance.BoxesController.Boxes.FirstOrDefault(box => box.X == (CurrentTile.X + x) && box.Y == (CurrentTile.Y + y));
            if (boxObj != null)
            {
    ...
    {
        if (CurrentTile.X + x < 0 || CurrentTile.Y + y < 0 || CurrentTile.X + x >= Main.Instance.SceneController.MapController.HorizontalTileCount || CurrentTile.Y + y >= Main.Instance.SceneController.MapController.VerticalTileCount)
            return false;
        return true;
    }
